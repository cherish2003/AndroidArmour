...
export const UploadFile = ({
  ...
  const handleUploadToAzure = async (file: File) => {
    try {
      const blobClient = containerClient.getBlockBlobClient(file.name);
      await blobClient.uploadData(file);
      return `https://${account}.blob.core.windows.net/${containerName}/${file.name}`;
    } catch (error) {
  ...
  const handleDecoupleApp = async (file: File) => {
    try {
      const fileUrl = await handleUploadToAzure(file);
      console.log("File uploaded to Azure at:", fileUrl);
      ...
      console.error("Error in decoupling app:", error);
    }
  };
  ...
  return (
    <div className="w-full" {...getRootProps()}>
      ...
      >
        ...
        <div className="flex flex-col items-center justify-center">
          ...
          <p className="relative z-20 font-sans font-normal text-neutral-400 dark:text-neutral-400 text-base mt-2">
            Drag or drop your apk here or click to upload
          </p>
      ...
      {fileUploadTriggered && (
        <Button variant="outline" className="mt-2 ml-96" onClick={fileupload}>
          Continue
        </Button>
      )}
    </div>
